...
{
  printf("Usage: dbn_roots -k <key> [-h] [-o <path>]\n");
  printf("\n");
  printf("Options:\n");
...
int main(int argc, char **argv)
{
  ...
  char *path = NULL;
  char c;
  while ((c = getopt(argc, argv, "hk:o:")) != -1)
  {
    switch(c)
    {
      case 'h':
  ...
  for (size_t i = 0; i < roots_count; i++)
  {
    ...
    while (d < n)
    {
      int r = write(fd, root + d, n - d);
      if (r <= 0)
      {
        printf("Failed to write roots : %s\n", strerror(errno));
        exit(EXIT_FAILURE);
      }
      else d += r;
    }

    const char suffix[] = ".OPT\n";
    n = strlen(suffix);
    d = 0;
    while (d < n)
    {
      int r = write(fd, suffix + d, n - d);
      if (r <= 0)
      {
        printf("Failed to write roots : %s\n", strerror(errno));
        exit(EXIT_FAILURE);
